...
def FindCirclesThres(current_frame_gray, num_of_rafts, radii_Hough = [17, 19], thres_value = 70, adaptiveThres_blocksize = 9, adaptiveThres_const = -20, sigma_Canny = 1.0, low_threshold_canny = 25, high_threshold_canny = 127, min_sep_dist = 20, lookup_radius = 768, raft_center_threshold = 60, topLeft_x = 390, topLeft_y = 450, width_x = 850, height_y = 850, error_message = ' '):
...
#    raft_radii[0] = radii[0]
    ...
    for accumScore, detected_cx, detected_cy, detected_radius in zip(accums, cx, cy, radii):
        ...
        if image_cropped[detected_cy, detected_cx] < raft_center_threshold: 
            new_raft = 0
        elif image_cropped[detected_cy-detected_radius//2 : detected_cy+detected_radius//2, detected_cx-detected_radius//2:detected_cx+detected_radius//2].mean() < raft_center_threshold:
            new_raft = 0
#        elif  (detected_cx - width_x/2)**2 +  (detected_cy - height_y/2)**2 > lookup_radius**2:
#            new_raft = 0   
        else:
            costMatrix = scipyDistance.cdist(np.array([detected_cx, detected_cy],ndmin=2), raft_centers[:raft_count,:], 'euclidean') 
            if np.any(costMatrix < min_sep_dist): # raft still exist
                new_raft = 0
        if new_raft == 1:
            raft_centers[raft_count, 0] = detected_cx # note that raft_count starts with 1, also note that cx corresonds to columns number
...
def FindCirclesAdaptive(current_frame_gray, num_of_rafts, radii_Hough = [17, 19], thres_value = 70, adaptiveThres_blocksize = 9, adaptiveThres_const = -20, sigma_Canny = 1.0, low_threshold_canny = 25, high_threshold_canny = 127, min_sep_dist = 20, lookup_radius = 768, raft_center_threshold = 60, topLeft_x = 390, topLeft_y = 450, width_x = 850, height_y = 850, error_message = ' '):
    ''' find the centers of each raft
    ...
    #threshold the image
    image_thres = cv.adaptiveThreshold(image_cropped,255,cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY,adaptiveThres_blocksize,adaptiveThres_const)
...
def FindAndSortCircles(image_gray, num_of_rafts, prev_pos, radii_Hough = [30, 40], thres_value = 30, sigma_Canny = 1.0, low_threshold_canny = 25, high_threshold_canny = 127, max_displ = 50):
    ...
    for raftID in np.arange(num_of_rafts):
        for accumScore, detected_cx, detected_cy, detected_radius in zip(accums, cx, cy, radii):
            ...
            if distance < max_displ:
                raft_centers[raftID, 0] = detected_cx # note that raft_count starts with 1, also note that cx corresonds to columns number
...
def NumberingRafts(rafts_loc, rafts_radii, num_of_rafts):
    ...
    orbiting_angles = np.arctan2(-(rafts_loc_sorted[:,1] - orbiting_center[1]), -(rafts_loc_sorted[:,0] - orbiting_center[0])) *180 / np.pi
    
    # concatenate and sort
    rafts_loc_radii_dist_angle_layer = np.column_stack((rafts_loc_sorted[:,0],rafts_loc_sorted[:,1], rafts_radii_sorted, dist_sorted, orbiting_angles,layer_index)) 
...
def CropImage(grayscale_image, raft_center, width):
    ...
    '''
    topRow = int(raft_center[1] - width/2) # note that y corresponds to rows, and is directed from top to bottom in scikit-image
...
def CountingEffusedRafts(prev_centers, prev_count, curr_centers, curr_count, boundary_x, max_displacement):
    ...
    costMatrix = scipyDistance.cdist(prev_centers[:prev_count], curr_centers[:curr_count], 'euclidean') 
    ...
    for raftID in np.arange(prev_count):
        if np.any(costMatrix[raftID,:] < max_displacement): # raft still exist
            curr_raftID = np.nonzero(costMatrix[raftID,:] < max_displacement)[0][0] # [0][0] is to convert array into scalar 
...
def GetRotationAngle(prev_image, curr_image):
    ...
    vector_along_x_axis_from_center = np.float32([[sizeOfCroppedRaftImage/2,sizeOfCroppedRaftImage/2], [sizeOfCroppedRaftImage, sizeOfCroppedRaftImage/2]]).reshape(-1,1,2)
    vector_transformed = cv.perspectiveTransform(vector_along_x_axis_from_center,transformMatrix)
    
    theta = - np.arctan2(vector_transformed[1,0,1] - vector_transformed[0,0,1], vector_transformed[1,0,0] - vector_transformed[0,0,0])*180/np.pi
...
#    line_length = 20
    ...
    for raft_id in np.arange(num_of_rafts):
        ...
        line_end = (int(rafts_loc[raft_id,0] + np.cos(rafts_ori[raft_id]*np.pi/180)*rafts_radii[raft_id]), int(rafts_loc[raft_id,1] - np.sin(rafts_ori[raft_id]*np.pi/180)*rafts_radii[raft_id]))
...
def DrawEffusedRaftCount(img_bgr, raft_effused, raft_to_left, raft_to_right, topLeft_X, topLeft_Y, width_X, height_Y):
    ...
    line_thickness = 1
    output_img = img_bgr
    output_img = cv.line(output_img,(topLeft_X + width_X//2, topLeft_Y),(topLeft_X + width_X//2, topLeft_Y + height_Y), line_color, line_thickness)
    output_img = cv.putText(output_img, 'Effused: ' + str(raft_effused), (topLeftX, topLeftY - 30), fontFace, font_scale, font_color, font_thickness, cv.LINE_AA)
    output_img = cv.putText(output_img, 'To left: ' + str(raft_to_left), (topLeftX, topLeftY - 60), fontFace, font_scale, font_color, font_thickness, cv.LINE_AA)
    output_img = cv.putText(output_img, 'To right: ' + str(raft_to_right), (topLeftX, topLeftY - 90), fontFace, font_scale, font_color, font_thickness, cv.LINE_AA)
...
for mainFolderID in np.arange(2,3): # note that np.arange(n) starts with 0, and ends with n-1. Remember half claose [) as numpy convention. 
    ...
    for expID in range(0, numOfExp):
        if isVideo == 1:
            ...
            numOfRafts, batchNum, spinSpeed, spinUnit, magnification, commentsSub = ParseSubfolderName(videoFileList[expID])
            ...
            numOfRafts, batchNum, spinSpeed, spinUnit, magnification, commentsSub = ParseSubfolderName(subfolders[expID])
        ...
        raftLocationsInRegion = np.zeros((maxNumOfRaftsInRegion, numOfFrames, 2), dtype=int) # (raftNum, frameNum, x(columns)&y(rows)
...
#            currentFrameGray = cv.imread(tiffFileList[currentFrameNum],0)
            currentFrameGray = currentFrameBGR[:,:,1] # use only green channel. We found green channel has the highest contrast. 
        ...
        centers, radii, prevCount = FindCirclesAdaptive(currentFrameGray, numOfRafts, radii_Hough = radiusIntervalHough, 
                                         thres_value = thresholdingValue,  adaptiveThres_blocksize = adaptiveThresBlocksize, 
                                         adaptiveThres_const = adaptiveThresConst,
                                         sigma_Canny = sigmaCanny, low_threshold_canny = lowThresholdCanny, 
                                         high_threshold_canny = highThresholdCanny, min_sep_dist = minSepDist, 
                                         lookup_radius = lookupRadius, raft_center_threshold = raftCenterThreshold,
                                         topLeft_x = topLeftX, topLeft_y = topLeftY, width_x = widthX, height_y = heightY, error_message = ' ')
        
        if regionalSearch == 1: 
             centersInRegion, radiiInRegion, countInRegion = FindCirclesAdaptive(currentFrameGray, maxNumOfRaftsInRegion, radii_Hough = radiusIntervalHough, 
                                                                 thres_value = thresholdingValue,  adaptiveThres_blocksize = adaptiveThresBlocksize, 
                                                                 adaptiveThres_const = adaptiveThresConst,
                                                                 sigma_Canny = sigmaCanny, low_threshold_canny = lowThresholdCanny, 
                                                                 high_threshold_canny = highThresholdCanny, min_sep_dist = minSepDist, 
                                                                 lookup_radius = lookupRadius, raft_center_threshold = raftCenterThreshold,
                                                                 topLeft_x = regionTopLeftX, topLeft_y = regionTopLeftY, width_x = regionWidth, 
                                                                 height_y = regionHeight, error_message = ' ')
...
#            continue
        ...
        # sorting
        centersSorted, radiiSorted, distSorted, orbitingAnglesSorted, layerIndexSorted = NumberingRafts(centers, radii, numOfRafts)
        ...
        if processRotation == 1:
            for raftID in np.arange(numOfRafts):
                firstImages[raftID,:,:] = CropImage(currentFrameGray,raftLocations[raftID,currentFrameNum,:], sizeOfCroppedRaftImage)
                raftOrientations[raftID,currentFrameNum] = raftInitialOrientation # this could change later when we have a external standard to define what degree is. 
        ...
        if effusionData == 1:
            currentFrameDraw = DrawEffusedRaftCount(currentFrameDraw, raftEffused[currentFrameNum], raftToLeft[currentFrameNum], raftToRight[currentFrameNum], topLeftX, topLeftY, widthX, heightY)
        if processRotation == 1:
            currentFrameDraw = DrawRaftOrientations(currentFrameDraw, raftLocations[:,currentFrameNum,:], raftOrientations[:,currentFrameNum], raftRadii[:,currentFrameNum], numOfRafts)
        if outputImageSeq == 1: 
            outputImageName = date + '_' + str(numOfRafts) + 'Rafts_' + str(batchNum) + '_' + str(spinSpeed) + 'rps_' + str(currentFrameNum+1).zfill(4) + '.jpg'
            cv.imwrite(outputImageName,currentFrameDraw)
        if outputVideo == 1: 
            outputVideoName = outputImageName = date + '_' + str(numOfRafts) + 'Rafts_' + str(batchNum) + '_' + str(spinSpeed) + 'rps_' + str(magnification) + 'x_' + commentsSub +'.mp4'
        ...
        for currentFrameNum in progressbar.progressbar(range(1, numOfFrames)): #Note that the first frame has been dealt with, so currentFrameNum = 0 is omitted
            if isVideo == 1: 
                retval, currentFrameBGR = cap.read()
                currentFrameGray = cv.cvtColor(currentFrameBGR, cv.COLOR_BGR2GRAY)
            else:
...
#                currentFrameGray = cv.imread(tiffFileList[currentFrameNum], 0)
            ...
            if diffusionData == 1:
                ...
                centers, radii, currCount = FindCirclesAdaptive(currentFrameGray, numOfRafts, radii_Hough = radiusIntervalHough,
                                                 thres_value = thresholdingValue, adaptiveThres_blocksize = adaptiveThresBlocksize, 
                                                 adaptiveThres_const = adaptiveThresConst,
                                                 sigma_Canny = sigmaCanny, low_threshold_canny = lowThresholdCanny, 
                                                 high_threshold_canny = highThresholdCanny, min_sep_dist = minSepDist, 
                                                 lookup_radius = lookupRadius, raft_center_threshold = raftCenterThreshold,
                                                 topLeft_x = diffBoxTopLeftX, topLeft_y = diffBoxTopLeftY, width_x = diffBoxWidthX, height_y = diffBoxHeightY, error_message = ' ')
...
#                continue
            ...
            if effusionData == 1:
                ...
                raftMovingToLeft, raftMovingToRight = CountingEffusedRafts(raftLocations[:,currentFrameNum-1,:], prevCount, centers, currCount, effusionBoundaryX, maxDisplacement)
...
#                raftOrbitingAngles[raftID,currentFrameNum] = CalculateOrbitingAngle(raftOrbitingCenters[currentFrameNum,:], raftLocations[raftID,currentFrameNum,:])
            ...
            if processRotation == 1:
                for raftID in np.arange(numOfRafts):
                    currImages[raftID,:,:] = CropImage(currentFrameGray,raftLocations[raftID,currentFrameNum,:], sizeOfCroppedRaftImage)
...
raft_centers, raft_radii, raft_count = FindCirclesThres(currentFrameGray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                       adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                       low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                       min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                       topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
print(raft_count)

## FindCircles function with adaptive thresholding
raft_centers, raft_radii, raft_count = FindCirclesAdaptive(currentFrameGray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                       adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                       low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                       min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                       topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
...
raft_centers, raft_radii, raft_count = FindAndSortCircles(currentFrameGray, num_of_rafts, prev_pos = prev_pos, 
                                                          ...
                                                          sigma_Canny = sigma_Canny, low_threshold_canny = low_threshold_canny, 
                                                          high_threshold_canny = high_threshold_canny, max_displ = max_displ)
...
curr_frame_draw = currentFrameBGR.copy()
curr_frame_draw = DrawRafts(curr_frame_draw, (currentFrameGray.shape[1]/2, currentFrameGray.shape[0]/2), lookup_radius, 1)
...
raft_centers_1, raft_radii_1, raft_count_1 = FindCirclesAdaptive(frame1Gray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                               adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                               low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                               min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                               topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
...
raft_centers_2, raft_radii_2, raft_count_2 = FindCirclesAdaptive(frame2Gray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                               adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                               low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                               min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                               topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
...
raftLocations = np.zeros((numOfRafts, 2, 2), dtype=int) # (raftNum, frameNum, x(columns)&y(rows), note that only two frames
...
frame1Draw = DrawRaftOrientations(frame1Draw, raftLocations[:,currentFrameNum,:], raftOrientations[:,currentFrameNum], raftRadii[:,currentFrameNum], numOfRafts)
plt.figure()
...
frame2Draw = DrawRaftOrientations(frame2Draw, raftLocations[:,currentFrameNum,:], raftOrientations[:,currentFrameNum], raftRadii[:,currentFrameNum], numOfRafts)
plt.figure()
...
for raftID in np.arange(numOfRafts):
    ...
    for indexX in np.arange(shiftingLength):
        for indexY in np.arange(shiftingLength):
            ...
            rollImagesTest[indexX, indexY] = np.abs(firstImages[raftID,:,:] - np.roll(currImages[raftID,:,:], (shiftInX, shiftInY), (0,1))).mean()
...
for counter, raftID in enumerate(rotatedRaftIDs[0]):
    if numOfRaftsSteppedOut == 1:
        ...
        ax[1].imshow(np.roll(currImages[raftID,:,:], (raftMatchingShifts[raftID, 1, 0], raftMatchingShifts[raftID, 1, 1]), (0,1)), 'gray')
    elif numOfRaftsSteppedOut > 1:
        ax[0,counter].imshow(firstImages[raftID,:,:], 'gray')
        ax[0,counter].set_title('raftID = {}'.format(raftID))
        ax[1,counter].imshow(np.roll(currImages[raftID,:,:], (raftMatchingShifts[raftID, 1, 0], raftMatchingShifts[raftID, 1, 1]), (0,1)), 'gray')
...
curr_image = np.roll(currImages[raftID,:,:], (raftMatchingShifts[raftID, 1, 0], raftMatchingShifts[raftID, 1, 1]), (0,1))
plt.figure()
plt.imshow(curr_image, 'gray') 
...
prev_centers, prev_radii, prev_count = FindCirclesThres(prevFrameGray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                           adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                           low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                           min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                           topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
## FindCircles in the current frame
curr_centers, curr_radii, curr_count = FindCirclesAdaptive(currFrameGray, num_of_rafts,  thres_value = thres_value, adaptiveThres_blocksize = adaptiveThres_blocksize, 
                                           adaptiveThres_const = adaptiveThres_const, radii_Hough = radii_Hough, sigma_Canny = sigma_Canny, 
                                           low_threshold_canny = low_threshold_canny, high_threshold_canny = high_threshold_canny, 
                                           min_sep_dist = min_sep_dist, lookup_radius = lookup_radius, raft_center_threshold = raft_center_threshold, 
                                           topLeft_x = topLeft_x, topLeft_y = topLeft_y, width_x = width_x, height_y = height_y, error_message = ' ')
